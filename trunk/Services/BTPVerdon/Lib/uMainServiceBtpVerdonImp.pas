...
procedure TSvcSyncBTPVerdonImp.ClearTablesValues;
begin
...
function TSvcSyncBTPVerdonImp.ReadSettings : boolean;
var
...
begin
  ...
  try
    ...
    try
      Reg.WriteString('Description', 'LSE-Synchronisation des donn�es entre BTP et VERDON - Import.');
    finally
      Reg.CloseKey;
    end;
  finally
...
procedure TSvcSyncBTPVerdonImp.ServiceExecute(Sender: TService);
  ...
  begin
    TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, lTn, '', LogValues, 0);
    TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, lTn, DupeString('*', 50), LogValues, 0);
    TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, lTn, TUtilBTPVerdon.GetMsgStartEnd(lTn, True, LastSynchro), LogValues, 0);
  end;

  procedure CallThreadTiers;
  begin
    if (LogValues.DebugEvents > 0) then TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, tnTiers, Format('%sWith Thread', [WSCDS_DebugMsg]), LogValues, 0);
    ...
    if (LogValues.DebugEvents > 0) then TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, tnTiers, Format('%sBefore Call uThreadTiers.Resume', [WSCDS_DebugMsg]), LogValues, 0);
    try
      uThreadTiers.Resume;
    except
      on E: Exception do
        LogMessage(Format('Fin ex�cution du service avec erreur : %s', [E.Message]), EVENTLOG_ERROR_TYPE);
    end;
  end;

  procedure CallThreadChantiers;
  begin
    if (LogValues.DebugEvents > 0) then TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, tnChantier, Format('%sWith Thread', [WSCDS_DebugMsg]), LogValues, 0);
    ...
    if (LogValues.DebugEvents > 0) then TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, tnChantier, Format('%sBefore Call uThreadChantier.Resume', [WSCDS_DebugMsg]), LogValues, 0);
    try
  ...
  begin
    if (LogValues.DebugEvents > 0) then TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, tnDevis, Format('%sWith Thread', [WSCDS_DebugMsg]), LogValues, 0);
    ...
    if (LogValues.DebugEvents > 0) then TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, tnDevis, Format('%sBefore Call uThreadDevis.Resume', [WSCDS_DebugMsg]), LogValues, 0);
    try
      uThreadDevis.Resume;
    except
      on E: Exception do
  ...
  begin
    if (LogValues.DebugEvents > 0) then TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, tnLignesBR, Format('%sWith Thread', [WSCDS_DebugMsg]), LogValues, 0);
    ...
    if (LogValues.DebugEvents > 0) then TUtilBTPVerdon.AddLog(ServiceName_BTPVerdonImp, tnLignesBR, Format('%sBefore Call uThreadLignesBR.Resume', [WSCDS_DebugMsg]), LogValues, 0);
    try
...
begin
  ...
  LogMessage(Format('Arr�t de %s.', [ServiceName_BTPVerdonImp]), EVENTLOG_INFORMATION_TYPE);
end;

procedure TSvcSyncBTPVerdonImp.ServiceStart(Sender: TService; var Started: Boolean);
begin
  LogMessage(Format('D�marrage de de %s.', [ServiceName_BTPVerdonImp]), EVENTLOG_INFORMATION_TYPE);
end;
