...
//  TConnectionSource = (Web, Outlook, Word, Excel, Scan, Custom, Mobile);
  ...
  public
    ...
    //
    function ConnectToServer (login: WideString; password: WideString): Boolean;
...
begin
  ...
  TRY
    ...
    if not XX.ConnectToServer(GetEmail(V_PGI.UserLogin),Currentpasswd) then Exit;
...
begin
  ...
  TRY
    ...
    Currentpasswd := AnsiLowerCase_(MD5(DeCryptageSt(V_PGI.Password)));
    Currentpasswd := FindEtReplace(Currentpasswd,'-','',true);
    if not XX.ConnectToServer(GetEmail(V_PGI.UserLogin),Currentpasswd) then
    begin
...
function TconnectBSV.ConnectToServer(login, password: WideString): Boolean;
