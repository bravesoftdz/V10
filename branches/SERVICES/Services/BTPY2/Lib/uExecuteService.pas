    ...
    try
      ...
      try
        ...
        begin
          ...
          //if LogValues.DebugEvents > 0 then TServicesLog.WriteLog(ssbylLog, Format('%s - Before TraGeneration.GetTraLine - Table name : %s - LineValue : %s', [WSCDS_DebugMsg, TableName, LineValue]), ServiceName_BTPY2, LogValues, 0);
...
begin
  case Tools.CaseFromString(TableName, [  Tools.GetTableNameFromTtn(ttnTiers)      , Tools.GetTableNameFromTtn(ttnSection) , Tools.GetTableNameFromTtn(ttnGeneraux)
                                        ...
                                        , Tools.GetTableNameFromTtn(ttnAcomptes)
                                       ]) of
...
begin                                                          
  ...
  begin
    ...
    try
      GetDataState := TReadWSDataService.GetData(wsdsAccForPayment, Y2Values.Server, Y2Values.DataBase, TSlValues, TSlCacheWSFields, LogValues, TSlFilter);
      Result := (GetDataState = WSCDS_GetDataOk);
      if Result then
      begin
        if TSlValues.Count > 0 then
    ...
    begin
      ...
      try
        ...
        case Y2EntriesNumber of
          ...
          0  : begin
                Msg := Format('Pièce %s - Erreur lors de l''envoi', [IntToStr(Cpt1)]);
                if ErrorMsg <> '' then
