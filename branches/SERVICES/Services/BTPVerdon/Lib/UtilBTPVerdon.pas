...
type
  ...
  private
    ...
    function GetTMPFieldInf(BTPFieldName : string; InfType : T_InfoTMPType) : string;
...
begin
  ...
  case Tn of
    ...
    tnArticles     : ArrLen := 19;
    tnReglement    : ArrLen := 20;
    tnHresSalaries : ArrLen := 19;
    tnConsoStock   : ArrLen := 14;
    tnIntervenants : ArrLen := 9;
  else
    Arrlen := 0;
  end;
    ...
    tnArticles :
      ...
      begin
        case Cpt of
          ...
          1  : AddValues('GA_TYPEARTICLE'  , 'ART_TYPEARTICLE');
          2  : AddValues('GA_NATUREPRES'   , 'ART_NATUREPRESTATION');
          3  : AddValues('GA_LIBELLE'      , 'ART_LIBELLE');
          4  : AddValues('GA_BLOCNOTE'     , 'ART_BLOCNOTE');
          5  : AddValues('GA_COMPTAARTICLE', 'ART_COMPTA');
          6  : AddValues('GA_DOMAINE'      , 'ART_DOMAINE');
          7  : AddValues('GA_FAMILLENIV1'  , 'ART_FAMILLENIV1');
          8  : AddValues('GA_FAMILLENIV2'  , 'ART_FAMILLENIV2');
          9  : AddValues('GA_FAMILLENIV3'  , 'ART_FAMILLENIV3');
          10 : AddValues('GA_LIBREART1'    , 'ART_MARQUE');
          11 : AddValues('GA_FERME'        , 'ART_ACTIF');
          12 : AddValues('GA_SOCIETE'      , 'ART_CODESOCIETE');
          13 : AddValues(''                , 'ART_DATECREATION');
          14 : AddValues(''                , 'ART_DATEMODIF');
          15 : AddValues(''                , 'ART_LOCK');
          16 : AddValues(''                , 'ART_TRAITE');
          17 : AddValues(''                , 'ART_DATETRAITE');
          18 : AddValues(''                , 'ART_ID');
        end;
      end;
    tnSalaries :
      ...
      begin
        case Cpt of
          ...
          8  : AddValues('ARS_UTILASSOCIE'  , 'SAL_CODEUTILISATEUR');
          9  : AddValues(''                 , 'SAL_CODESOCIETE');
          10 : AddValues(''                 , 'SAL_DATECREATION');
          11 : AddValues(''                 , 'SAL_DATEMODIF');
          12 : AddValues(''                 , 'SAL_LOCK');
          13 : AddValues(''                 , 'SAL_TRAITE');
          14 : AddValues(''                 , 'SAL_DATETRAITE');
          15 : AddValues(''                 , 'SAL_ID');
        end;
      end;
    tnModeRegle :
    ...
    tnIntervenants :
      ...
      begin
        case Cpt of
          ...
          4  : AddValues('AFT_TYPEINTERV'    , 'INT_TYPEINTERVENANT');
          5  : AddValues('ARS_FERME'         , 'INT_ACTIF');
          6  : AddValues(SocietyCodeFieldName, 'INT_CODESOCIETE');
          7  : AddValues('AFT_DATECREATION'  , 'INT_DATECREATION');
          8  : AddValues('AFT_DATEMODIF'     , 'INT_DATEMODIF');
        end;
      end;
  end;
end;
...
function TImportExportTreatment.GetTMPFieldInf(BTPFieldName : string; InfType : T_InfoTMPType) : string;
var
  Cpt : integer;
begin
  if BTPFieldName <> '' then
...
begin
  ...
  case Tn of
    ...
    , tnModeRegle
    , tnReglement
    , tnHresSalaries : Result := Format('SELECT %s FROM %s WHERE %s_LOCK = 0 AND %s_TRAITE = 0', [AdoQryTMP.FieldsList, TUtilBTPVerdon.GetTMPTableName(Tn), GetTMPPrefix, GetTMPPrefix]);
  else
    Result := '';
  end;
end;
...
var
  ...
  IniFilePath : string;
begin
  IniFilePath := Format('%s\%s.%s', [TServicesLog.GetServicesAppDataPath(True, 'VERDON'), ServiceName_BTPVerdonIniFile, 'ini']);
...
begin
  ...
  begin
    ...
    begin
      ...
      FieldSize     := GetTMPFieldSizeMax(GetTMPFieldInf(KeyField1, tittName));
      KeyValue1     := Tools.iif(FieldSize > -1, Trim(Copy(KeyValue1, 1, FieldSize)), KeyValue1);
      if KeyField2 <> '' then
      begin
        KeyValue2 := TobL.GetString(KeyField2);
        FieldSize := GetTMPFieldSizeMax(GetTMPFieldInf(KeyField2, tittName));
      ...
      begin
        ...
        begin  
          ...
          GetErrorMsgFieldType(KeyValue2, GetTMPFieldInf(KeyField2, tittName), GetTMPFieldInf(KeyField2, tittType));
      end else
        GetErrorMsgFieldType(KeyValue1, GetTMPFieldInf(KeyField1, tittName), GetTMPFieldInf(KeyField1, tittType));
    end;
    if FindData then
    begin
      TobL      := TOB.Create('_QRYL', TobQry, -1);
    ...
    tnArticles :
      Result := Format('  GA_NATUREPRES    = ''%s'''
                     ...
                     , [  TobDataL.GetString('ART_NATUREPRESTATION')
                        ...
                        , TobDataL.GetString('ART_DOMAINE')
...
begin
  ...
  case Tn of
            ...
            case Tools.CaseFromString(TypeParam, ['CHOIXCOD', 'CHOIXEXT', 'FONCTION']) of
              ...
              begin
                case Tools.CaseFromString(FieldName, ['CC_TYPE', 'YX_TYPE', 'CC_CODE', 'YX_CODE', 'CC_LIBELLE', 'YX_LIBELLE', 'CC_ABREGE', 'YX_ABREGE']) of
                  ...
                  {xx_LIBELLE} 4..5 : FieldValue := Format('''%s''', [TobDataL.GetString('PAR_LIBELLE')]);
                  {xx_ABREGE}  6..7 : FieldValue := Format('''%s''', [TobDataL.GetString('PAR_INFOCOMPL')]);
                else
                  FieldValue := '''''';
                end;
              end;
              ...
              begin
                case Tools.CaseFromString(FieldName, ['AFO_FONCTION', 'AFO_LIBELLE']) of
                  ...
                  {AFO_LIBELLE}  1 : FieldValue := Format('''%s''', [TobDataL.GetString('PAR_LIBELLE')]);
                else
    ...
    tnArticles :
      begin
        TypeParam   := TobDataL.GetString('ART_TYPEARTICLE');
        ...
        begin
          ...
          case Tools.CaseFromString(FieldName, [  'GA_ARTICLE'        , 'GA_TYPEARTICLE'    , 'GA_CODEARTICLE'     , 'GA_NATUREPRES'     , 'GA_LIBELLE'
            ...
            {GA_LIBELLE}          4  : FieldValue := Format('''%s''', [TobDataL.GetString('ART_LIBELLE')]);
            {GA_BLOCNOTE}         5  : FieldValue := Format('''%s''', [TobDataL.GetString('ART_BLOCNOTE')]);
      ...
      begin
        ...
        begin
          ...
          case Tools.CaseFromString(FieldName, [  'ARS_RESSOURCE'    , 'ARS_TYPERESSOURCE', 'ARS_LIBELLE'  , 'ARS_LIBELLE2'   , 'ARS_FONCTION1'
            ...
            {ARS_LIBELLE}       2  : FieldValue := Format('''%s''', [TobDataL.GetString('SAL_NOM')]);
            {ARS_LIBELLE2}      3  : FieldValue := Format('''%s''', [TobDataL.GetString('SAL_PRENOM')]);
      ...
      begin
        ...
        begin
          ...
          case Tools.CaseFromString(FieldName, ['MP_MODEPAIE', 'MP_LIBELLE', 'MP_CATEGORIE']) of
            ...
            {MP_LIBELLE}   1  : FieldValue := Format('''%s''', [TobDataL.GetString('MDR_LIBELLE')]);
        ...
        begin
          ...
          begin
            ...
            case Tools.CaseFromString(FieldName, [  'GAC_NATUREPIECEG', 'GAC_SOUCHE'    , 'GAC_NUMERO'      , 'GAC_INDICEG'   , 'GAC_JALECR'
              ...
              {GAC_LIBELLE}      8  : FieldValue := Format('''%s''', [TobDataL.GetString('EDR_LIBELLE')]);
    ...
    tnHresSalaries :
      begin
        ...
        begin
          lFieldsList := HresSalariesFieldsList;
          while lFieldsList <> '' do
          begin
            FieldName := Tools.ReadTokenSt_(lFieldsList, ',');
            case Tools.CaseFromString(FieldName, [  'BCO_DATEMOUV', 'BCO_AFFAIRE1' , 'BCO_RESSOURCE', 'BCO_LIBELLE', 'BCO_CODEARTICLE'
                                                  , 'BCO_QUANTITE', 'BCO_TYPEHEURE', 'BCO_DPA'      , 'BCO_DPR'    , 'BCO_PUHT'
                                                  , 'BCO_NUMMOUV'
                                                 ]) of
              {BCO_DATEMOUV}    0  : FieldValue := Format('''%s''', [Tools.CastDateTimeForQry(TobDataL.GetDateTime('HSA_DATEMVT'))]);
              {BCO_AFFAIRE1}    1  : FieldValue := Format('''%s''', [TobDataL.GetString('HSA_CODECHA')]);
              {BCO_RESSOURCE}   2  : FieldValue := Format('''%s''', [TobDataL.GetString('HSA_CODESAL')]);
              {BCO_LIBELLE}     3  : FieldValue := Format('''%s''', [TobDataL.GetString('HSA_LIBELLE')]);
              {BCO_CODEARTICLE} 4  : FieldValue := Format('''%s''', [TobDataL.GetString('HSA_CODEPRESTATION')]);
              {BCO_QUANTITE}    5  : FieldValue := Format('''%s''', [FloatToStr(Valeur(TobDataL.GetString('HSA_NBHRS')))]);
              {BCO_TYPEHEURE}   6  : FieldValue := Format('''%s''', [TobDataL.GetString('HSA_TYPEHRS')]);
              {BCO_DPA}         7  : FieldValue := Format('''%s''', [FloatToStr(Valeur(TobDataL.GetString('HSA_DPA')))]);
              {BCO_DPR}         8  : FieldValue := Format('''%s''', [FloatToStr(Valeur(TobDataL.GetString('HSA_DPR')))]);
              {BCO_PUHT}        9  : FieldValue := Format('''%s''', [FloatToStr(Valeur(TobDataL.GetString('HSA_PUV')))]);
              {BCO_NUMMOUV}     10 : FieldValue := Format('''%s''', [FloatToStr(MvtNumber)]);
            else
              FieldValue := GetBTPDefaultValue(FieldName);
            end;
            Result := Format('%s, %s', [Result, FieldValue]);
  ...
  begin
    ...
    case Tn of
      ...
      tnArticles :
        begin
          case Pos of
            ...
            3 : Result := TobDataL.GetString('ART_TYPEARTICLE');
          end;
        end;
      tnSalaries :
        begin
      ...
      tnHresSalaries :
        begin
          case Pos of
            1 : Result := TobDataL.GetString('HSA_IDENTIFIANT');
            2 : Result := TobDataL.GetString('HSA_CODECHANTIER');
            3 : Result := TobDataL.GetString('HSA_CODESAL');
          end;
        end;
    end;
  end;
...
begin
  ...
  { Ajout de la piéce comptable dans le fichier }
  ...
  begin
    ...
    if TobL.GetString('E_TYPEMVT') = 'HT' then // Il faut exporter QUE les ligne de HT
    begin
      Inc(LineNumber);
...
begin
  ...
  else if CustomerCode = '' then
    PGIError('Export impossible, le code client EXACT n''est pas renseigné.', MsgCaption)
  else
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          TslData.Add('');
          TslData.Add('Pièces comptable (Exercice - Journal - Numéro de pièce) :');
          BeginTrans;
          try
